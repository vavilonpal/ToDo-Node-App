...
- JSON Web Token (JWT) — это компактный токен для передачи информации о пользователе между клиентом и сервером.
Содержит закодированный payload (например, userId и роль) и подпись. Сервер проверяет подпись, 
чтобы удостовериться, что данные не были изменены.
2. Как реализовать безопасное хранение паролей пользователей?
- Использовать хэширование паролей с солью (например, bcrypt). Никогда не хранить пароли в открытом виде.
При аутентификации сравнивать хэш введённого пароля с сохранённым хэшем.
3. В чём разница между аутентификацией и авторизацией?
- Аутентификация — проверка личности пользователя (кто он).
- Авторизация — проверка прав пользователя (что ему разрешено делать).
4. Какие преимущества и недостатки использования Passport.js для аутентификации в Node.js
   **Преимущества:** большое количество стратегий, удобная интеграция, стандартизованный подход.  
   **Недостатки:** может быть сложным для простых проектов, требует изучения концепции middleware и стратегий.